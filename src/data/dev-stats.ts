/* --------------------------------------------------------------------------
   ...
   which days it landed on. Both are public, both are read once a day.

   Server-only: this module fetches. Import it from a server component and hand
   the plain result down as props, never from a `"use client"` file.
   -------------------------------------------------------------------------- */
...
/** Both sources are re-read once a day; a portfolio does not need fresher. */
...
/**
 * Read once per revalidation window, however many pages ask for it. Any
 * failure — network, non-2xx, malformed JSON, renamed GitHub markup — serves
 * the frozen year instead, flagged `stale` so the section can say so.
 */
